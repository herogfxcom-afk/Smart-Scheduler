...
async def is_user_in_chat(chat_id: str, user_telegram_id: int) -> bool:
    ...
    try:
        ...
        result = True # Soft fail on network error
...
async def sync_group(data: dict, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    if membership_status == "bot_not_in_chat":
        raise HTTPException(status_code=400, detail=f"Bot is not a member of this group. Please add @{BOT_USERNAME_FALLBACK} to the chat.")
...
async def update_meeting(meeting_id: int, data: dict, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    if "start" in data: 
        ...
        meeting.start_time = datetime.fromisoformat(s_raw).astimezone(ZoneInfo("UTC")).replace(tzinfo=None)
    if "end" in data: 
        e_raw = data["end"].replace('Z', '+00:00')
        meeting.end_time = datetime.fromisoformat(e_raw).astimezone(ZoneInfo("UTC")).replace(tzinfo=None)
